...
def main(args):
    ...
    for input, output in zip(train_input, train_output):
        full_sequence = input + '\n' + output
        # tokenized_example = tokenizer(input, text_target=output, max_length=1024, padding="max_length", truncation=True)
        tokenized_example = tokenizer(full_sequence, text_target=full_sequence, max_length=1024, padding="max_length", truncation=True)
    ...
    for input, output in zip(validation_input, validation_output):
        # tokenized_example = tokenizer(input, text_target=output, max_length=100, padding="max_length", truncation=True)
        full_sequence = input + '\n' + output
        tokenized_example = tokenizer(full_sequence, text_target=full_sequence, max_length=1024, padding="max_length", truncation=True)
